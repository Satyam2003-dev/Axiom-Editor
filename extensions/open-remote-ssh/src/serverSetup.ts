...
const DEFAULT_DOWNLOAD_URL_TEMPLATE = 'https://github.com/Satyam2003-dev/Axiom-Editor/releases/download/${release}/axiom-reh-${os}-${arch}-${version}.tar.gz';
...
SERVER_DOWNLOAD_URL="$(echo "${serverDownloadUrlTemplate.replace(/\$\{/g, '\\${')}" | sed "s/\\\${quality}/$DISTRO_QUALITY/g" | sed "s/\\\${version}/$DISTRO_VERSION/g" | sed "s/\\\${commit}/$DISTRO_COMMIT/g" | sed "s/\\\${os}/$PLATFORM/g" | sed "s/\\\${arch}/$SERVER_ARCH/g" | sed "s/\\\${release}/$DISTRO_AXIOM_RELEASE/g")"
