...
# Configuration
APP_NAME="axiom-editor"
APP_VERSION="1.0.0"
ARCH="x86_64"

export ARCH

# Check if axiom-editor binary exists in current directory
if [ ! -f "./axiom-editor" ]; then
    echo "Error: axiom-editor binary not found in current directory"
    exit 1
fi

# Check if icon exists
if [ ! -f "./axiom.png" ]; then
    echo "Error: axiom.png icon not found in current directory"
    exit 1
fi
...
cp -v ./axiom-editor "$APP_DIR/usr/bin/"

# Copy the icon to required locations
cp -v ./axiom.png "$APP_DIR/axiom.png"
cp -v ./axiom.png "$APP_DIR/usr/share/icons/hicolor/256x256/apps/axiom.png"

# Copy dependencies with error checking
echo "Copying dependencies..."
for lib in $(ldd ./axiom-editor | grep "=> /" | awk '{print $3}'); do
    if [ -f "$lib" ]; then
...
Type=Application
Categories=Utility;
Comment=Axiom Linux Application
EOF
then
