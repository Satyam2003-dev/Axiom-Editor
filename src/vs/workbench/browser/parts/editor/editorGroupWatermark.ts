...
/* eslint-disable */ // Axiom
import { AXIOM_CTRL_K_ACTION_ID, AXIOM_CTRL_L_ACTION_ID } from '../../../contrib/axiom/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const axiomIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			// Axiom - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				axiomIconBox.appendChild(buttonContainer);

				// Open a folder
				const openFolderButton = h('button')
				openFolderButton.root.classList.add('axiom-openfolder-button')
				...
				openSSHButton.root.classList.add('axiom-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					axiomIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('axiom-link')
				...
				// show them Axiom keybindings
				const keys = this.keybindingService.lookupKeybinding(AXIOM_CTRL_L_ACTION_ID);
				const dl = append(axiomIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(AXIOM_CTRL_K_ACTION_ID);
				const dl2 = append(axiomIconBox, $('dl'));
