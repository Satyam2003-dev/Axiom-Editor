...
import { mountAxiomCommandBar } from './react/out/axiom-editor-widgets-tsx/index.js'
...
import { AXIOM_ACCEPT_DIFF_ACTION_ID, AXIOM_REJECT_DIFF_ACTION_ID, AXIOM_GOTO_NEXT_DIFF_ACTION_ID, AXIOM_GOTO_PREV_DIFF_ACTION_ID, AXIOM_GOTO_NEXT_URI_ACTION_ID, AXIOM_GOTO_PREV_URI_ACTION_ID, AXIOM_ACCEPT_FILE_ACTION_ID, AXIOM_REJECT_FILE_ACTION_ID, AXIOM_ACCEPT_ALL_DIFFS_ACTION_ID, AXIOM_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IAxiomCommandBarService {
...
export const IAxiomCommandBarService = createDecorator<IAxiomCommandBarService>('AxiomCommandBarService');
...
export class AxiomCommandBarService extends Disposable implements IAxiomCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'axiom.AxiomCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._axiomModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IAxiomCommandBarService, AxiomCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type AxiomCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountAxiomCommandBar(root, accessor, { uri, editor } satisfies AxiomCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies AxiomCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('axiomAcceptDiffAction', 'Axiom: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.AxiomExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IAxiomCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('axiomRejectDiffAction', 'Axiom: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('axiomGoToNextDiffAction', 'Axiom: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('axiomGoToPrevDiffAction', 'Axiom: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('axiomGoToNextUriAction', 'Axiom: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('axiomGoToPrevUriAction', 'Axiom: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('axiomAcceptFileAction', 'Axiom: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('axiomRejectFileAction', 'Axiom: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('axiomAcceptAllDiffsAction', 'Axiom: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('axiomRejectAllDiffsAction', 'Axiom: Reject All Diffs in All Files'),
			keybinding: {
