...
import { mountAxiomOnboarding } from './react/out/axiom-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.axiomOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.axiom-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountAxiomOnboarding(onboardingContainer, accessor);
