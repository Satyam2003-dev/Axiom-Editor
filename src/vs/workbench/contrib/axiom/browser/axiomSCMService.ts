...
import { IAxiomSCMService } from '../common/axiomSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IAxiomSettingsService } from '../common/axiomSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/axiomSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('axiomGenerateCommitMessageService');

const loadingContextKey = 'axiomSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private axiomSCM: IAxiomSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IAxiomSettingsService private readonly axiomSettingsService: IAxiomSettingsService,
		...
		this.axiomSCM = ProxyChannel.toService<IAxiomSCMService>(mainProcessService.getChannel('axiom-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.axiomSCM.gitStat(path),
					this.axiomSCM.gitSampledDiffs(path),
					this.axiomSCM.gitBranch(path),
					this.axiomSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.axiomSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.axiomSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.axiomSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'AxiomSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('axiomFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'axiom.generateCommitMessageAction',
			title: localize2('axiomCommitMessagePrompt', 'Axiom: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('axiomCommitMessagePromptTooltip', 'Axiom: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'axiom.loadingGenerateCommitMessageAction',
			title: localize2('axiomCommitMessagePromptCancel', 'Axiom: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('axiomCommitMessagePromptCancelTooltip', 'Axiom: Cancel Commit Message Generation'),
