...
import { mountAxiomSelectionHelper } from './react/out/axiom-editor-widgets-tsx/index.js';
...
export type AxiomSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.axiomSelectionHelper';
	// react
	...
	constructor(
		...
		@IAxiomSettingsService private readonly _axiomSettingsService: IAxiomSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountAxiomSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._axiomSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies AxiomSelectionHelperProps)
