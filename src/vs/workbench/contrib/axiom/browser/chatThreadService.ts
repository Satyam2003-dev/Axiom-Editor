...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/axiomSettingsTypes.js';
...
import { AxiomFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('axiomChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Axiom)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Axiom), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const axiomFileSnapshot = checkpointMessage.axiomFileSnapshotOfURI ? checkpointMessage.axiomFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { axiomFileSnapshot, } }

		const userModifiedAxiomFileSnapshot = fsPath in checkpointMessage.userModifications.axiomFileSnapshotOfURI ? checkpointMessage.userModifications.axiomFileSnapshotOfURI[fsPath] ?? null : null
		return { axiomFileSnapshot: userModifiedAxiomFileSnapshot ?? axiomFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const axiomFileSnapshotOfURI: { [fsPath: string]: AxiomFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._axiomModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { axiomFileSnapshot: oldAxiomFileSnapshot } = res
			...
			const axiomFileSnapshot = this._editCodeService.getAxiomFileSnapshot(URI.file(fsPath))
			if (oldAxiomFileSnapshot === axiomFileSnapshot) continue
			axiomFileSnapshotOfURI[fsPath] = axiomFileSnapshot
		}
		...
		return { axiomFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { axiomFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			axiomFileSnapshotOfURI: axiomFileSnapshotOfURI ?? {},
			userModifications: { axiomFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getAxiomFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			axiomFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { axiomFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.axiomFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { axiomFileSnapshotOfURI: axiomFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { axiomFileSnapshot } = res
					if (!axiomFileSnapshot) continue
					this._editCodeService.restoreAxiomFileSnapshot(URI.file(fsPath), axiomFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._axiomModelService.getModelSafe(uri)
