...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/axiomSettingsTypes.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .axiomrules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IAxiomModelService private readonly axiomModelService: IAxiomModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .axiomrules files from workspace folders
	private _getAxiomRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let axiomRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.axiomrules')
				const { model } = this.axiomModelService.getModel(uri)
				if (!model) continue
				axiomRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return axiomRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .axiomrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.axiomSettingsService.state.globalSettings.aiInstructions;
		const axiomRulesFileContent = this._getAxiomRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (axiomRulesFileContent) ans.push(axiomRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.axiomSettingsService.state
		...
		const modelSelectionOptions = this.axiomSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.axiomSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.axiomSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
