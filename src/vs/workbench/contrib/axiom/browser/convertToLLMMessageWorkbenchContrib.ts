...
class ConvertContribWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.axiom.convertcontrib'
	...
	constructor(
		...
		const initializeURI = (uri: URI) => {
			...
			const axiomRulesURI = URI.joinPath(uri, '.axiomrules')
			this.axiomModelService.initializeModel(axiomRulesURI)
		}
