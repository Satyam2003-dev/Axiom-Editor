...
import { axiomPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IAxiomCommandBarService } from './axiomCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { AXIOM_ACCEPT_DIFF_ACTION_ID, AXIOM_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { IAxiomSettingsService } from '../common/axiomSettingsService.js';
import { FeatureName } from '../common/axiomSettingsTypes.js';
import { IAxiomModelService } from '../common/axiomModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, AxiomFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IAxiomSettingsService private readonly _settingsService: IAxiomSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IAxiomModelService private readonly _axiomModelService: IAxiomModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._axiomModelService.initializeModel(model.uri)
	...
	// 		message: `Axiom Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'axiom.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Axiom's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(AXIOM_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/Satyam2003-dev/Axiom-Editor/issues/new/choose) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._axiomModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'axiom-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'axiom-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'axiom-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'axiom-greenBG', {
	...
	private _getCurrentAxiomFileSnapshot = (uri: URI): AxiomFileSnapshot => {
		const { model } = this._axiomModelService.getModel(uri)
	...
	private _restoreAxiomFileSnapshot = async (uri: URI, snapshot: AxiomFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: AxiomFileSnapshot = this._getCurrentAxiomFileSnapshot(uri)
		let afterSnapshot: AxiomFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreAxiomFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreAxiomFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentAxiomFileSnapshot(uri)
			await this._axiomModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getAxiomFileSnapshot(uri: URI) {
		return this._getCurrentAxiomFileSnapshot(uri)
	}


	public restoreAxiomFileSnapshot(uri: URI, snapshot: AxiomFileSnapshot): void {
		this._restoreAxiomFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Axiom: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._axiomModelService.initializeModel(uri)
		await this._axiomModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Axiom: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = axiomPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Axiom: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Axiom error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IAxiomCommandBarService private readonly _axiomCommandBarService: IAxiomCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(AXIOM_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(AXIOM_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._axiomCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._axiomCommandBarService.onDidChangeState(e => {
