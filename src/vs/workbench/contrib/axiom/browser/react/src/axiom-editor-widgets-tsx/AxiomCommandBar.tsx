...
import { AxiomCommandBarProps } from '../../../axiomCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	AXIOM_GOTO_NEXT_DIFF_ACTION_ID,
	AXIOM_GOTO_PREV_DIFF_ACTION_ID,
	AXIOM_GOTO_NEXT_URI_ACTION_ID,
	AXIOM_GOTO_PREV_URI_ACTION_ID,
	AXIOM_ACCEPT_FILE_ACTION_ID,
	AXIOM_REJECT_FILE_ACTION_ID,
	AXIOM_ACCEPT_ALL_DIFFS_ACTION_ID,
	AXIOM_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const AxiomCommandBarMain = ({ uri, editor }: AxiomCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@axiom-scope ${isDark ? 'dark' : ''}`}
	>
		<AxiomCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const AxiomCommandBar = ({ uri, editor }: AxiomCommandBarProps) => {
	...
	const _upKeybinding = keybindingService.lookupKeybinding(AXIOM_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(AXIOM_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(AXIOM_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(AXIOM_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(AXIOM_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(AXIOM_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(AXIOM_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(AXIOM_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-axiom-bg-2 shadow-md border border-axiom-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-axiom-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-axiom-bg-2 rounded shadow-md border border-axiom-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-axiom-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-axiom-bg-2 rounded shadow-md border border-axiom-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-axiom-border-2 [&>*:last-child]:border-r-0">
