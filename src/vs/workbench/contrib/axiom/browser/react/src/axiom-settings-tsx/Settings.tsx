...
import { ProviderName, SettingName, displayInfoOfSettingName, providerNames, AxiomStatefulModelInfo, customSettingNamesOfProvider, RefreshableProviderName, refreshableProviderNames, displayInfoOfProviderName, nonlocalProviderNames, localProviderNames, GlobalSettingName, featureNames, displayInfoOfFeatureName, isProviderNameDisabled, FeatureName, hasDownloadButtonsOnModelsProviderNames, subTextMdOfProviderName } from '../../../../common/axiomSettingsTypes.js'
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js'
import { AxiomButtonBgDarken, AxiomCustomDropdownBox, AxiomInputBox2, AxiomSimpleInputBox, AxiomSwitch } from '../util/inputs.js'
...
const ButtonLeftTextRightOption = ({ text, leftButton }: { text: string, leftButton?: React.ReactNode }) => {
	...
	return <div className='flex items-center text-axiom-fg-3 px-3 py-0.5 rounded-sm overflow-hidden gap-2'>
...
const ConfirmButton = ({ children, onConfirm, className }: { children: React.ReactNode, onConfirm: () => void, className?: string }) => {
	...
	return (
		<div ref={ref} className={`inline-block`}>
			<AxiomButtonBgDarken className={className} onClick={() => {
				if (!confirm) {
					setConfirm(true);
				} else {
					onConfirm();
...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('IAxiomSettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/Satyam2003-dev/Axiom-Editor/blob/main/src/vs/workbench/contrib/axiom/common/modelCapabilities.ts`
	...
	return (
		...
		>
			{/* MODAL */}
			<div
				className="bg-axiom-bg-1 rounded-md p-4 max-w-xl w-full shadow-xl overflow-y-auto max-h-[90vh]"
			...
			>
				<div className="flex justify-between items-center mb-4">
					...
					<button
						onClick={onClose}
						className="text-axiom-fg-3 hover:text-axiom-fg-1"
					>
						<X className="size-5" />
					</button>
				</div>
				...
				<div className="text-sm text-axiom-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Axiom, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Axiom.`
							: `Axiom recognizes ${modelName} ("${recognizedModelName}").`}
				</div>


				{/* override toggle */}
				<div className="flex items-center gap-2 mb-4">
					<AxiomSwitch size='xs' value={overrideEnabled} onChange={setOverrideEnabled} />
					<span className="text-axiom-fg-3 text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="text-sm text-axiom-fg-3 mb-4">
				...
				<textarea
					key={overrideEnabled + ''}
					ref={textAreaRef}
					className={`w-full min-h-[200px] p-2 rounded-sm border border-axiom-border-2 bg-axiom-bg-2 resize-none font-mono text-sm ${!overrideEnabled ? 'text-axiom-fg-3' : ''}`}
				...
				<div className="flex justify-end gap-2 mt-4">
					<AxiomButtonBgDarken onClick={onClose} className="px-3 py-1">
						Cancel
					</AxiomButtonBgDarken>
					<AxiomButtonBgDarken
						onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	const modelDump: (AxiomStatefulModelInfo & { providerName: ProviderName, providerEnabled: boolean })[] = []
	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='axiom-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='axiom-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
				...
				<div className="flex items-center gap-2 w-fit">
					...
					<div className={`w-5 flex items-center justify-center`}>
						{type === 'default' || type === 'autodetected' ? null : <button
							...
							className={`${hasOverrides ? '' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
						>
							<X size={12} className="text-axiom-fg-3 opacity-50" />
						</button>}
					</div>
				</div>
			</div>
		...
		{showCheckmark ? (
				...
				<form className="flex items-center gap-2">
					...
					<ErrorBoundary>
						<AxiomCustomDropdownBox
							...
							className="max-w-32 mx-2 w-full resize-none bg-axiom-bg-1 text-axiom-fg-1 placeholder:text-axiom-fg-3 border border-axiom-border-2 focus:border-axiom-border-1 py-1 px-2 rounded"
			...
			<div
				className="text-axiom-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer mt-4"
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const accessor = useAccessor()
	const axiomSettingsService = accessor.get('IAxiomSettingsService')
	const settingsState = useSettingsState()
	...
	const handleChangeValue = useCallback((newVal: string) => {
		axiomSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [axiomSettingsService, providerName, settingName]);
...
// 	const axiomSettingsState = useSettingsState()

// 	const needsModel = isProviderNameDisabled(providerName, axiomSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const axiomSettingsService = accessor.get('IAxiomSettingsService')

// 	// const { enabled } = axiomSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = axiomSettingsService.state.settingsOfProvider[providerName].enabled
// 						axiomSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [axiomSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const AxiomProviderSettings = ({ providerNames }: { providerNames: ProviderName[] }) => {
	return <>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = axiomSettingsState.globalSettings[settingName]

	return <ButtonLeftTextRightOption
		leftButton={<AxiomSwitch
			size='xxs'
			value={enabled}
			onChange={(newVal) => {
				axiomSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	const axiomSettingsService = accessor.get('IAxiomSettingsService')
	const axiomSettingsState = useSettingsState()
	return <AxiomInputBox2
		className='min-h-[81px] p-3 rounded-sm'
		initValue={axiomSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			axiomSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		axiomSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [axiomSettingsService])

	return <AxiomCustomDropdownBox
		className='text-xs text-axiom-fg-3 bg-axiom-bg-1 border border-axiom-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={axiomSettingsService.state.globalSettings.enableFastApply}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-axiom-fg-3 text-sm list-decimal select-text'>
		<div className=''><ChatMarkdownRender string={`Ollama Setup Instructions`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`1. Download [Ollama](https://ollama.com/download).`} chatMessageLocation={undefined} /></div>
		<div className=' pl-6'><ChatMarkdownRender string={`2. Open your terminal.`} chatMessageLocation={undefined} /></div>
		<div
			className='pl-6 flex items-center w-fit'
			data-tooltip-id='axiom-tooltip-ollama-settings'
		>
			<ChatMarkdownRender string={`3. Run \`ollama pull your_model\` to install a model.`} chatMessageLocation={undefined} />
		</div>
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Axiom automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const axiomSettingsService = accessor.get('IAxiomSettingsService')
	return <div
		className={`text-axiom-fg-4 flex flex-nowrap text-nowrap items-center hover:brightness-110 cursor-pointer ${className}`}
		onClick={() => { axiomSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		axiomSettingsService.setGlobalSetting('autoApprove', {
			...axiomSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [axiomSettingsService, metricsService])

	return <>
		<AxiomSwitch
			size={size}
			value={axiomSettingsState.globalSettings.autoApprove[approvalType] ?? false}
			onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)}
		/>
		<span className="text-axiom-fg-3 text-xs">{desc}</span>
	</>
}
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: { fromEditor?: TransferEditorType, className?: string }) => {
	...
	return <>
		<AxiomButtonBgDarken className={`max-w-48 p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const axiomSettings = useSettingsState()
	const isOn = axiomSettings.mcpUserStateOfName[name]?.isOn

	const removeUniquePrefix = (name: string) => name.split('_').slice(1).join('_')

	return (
		<div className="border border-axiom-border-2 bg-axiom-bg-1 py-3 px-4 rounded-sm my-2">
						...
						${server.status === 'success' ? 'bg-green-500'
							: server.status === 'error' ? 'bg-red-500'
								: server.status === 'loading' ? 'bg-yellow-500'
									: server.status === 'offline' ? 'bg-axiom-fg-3'
										: ''}
					`}></div>

					{/* Server name */}
					<div className="text-sm font-medium text-axiom-fg-1">{name}</div>
				</div>
					...
					<div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
						{(server.tools ?? []).length > 0 ? (
							(server.tools ?? []).map((tool: { name: string; description?: string }) => (
								<span
									key={tool.name}
									className="px-2 py-0.5 bg-axiom-bg-2 text-axiom-fg-3 rounded-sm text-xs"

									data-tooltip-id='axiom-tooltip'
									data-tooltip-content={tool.description || ''}
									data-tooltip-class-name='axiom-max-w-[300px]'
								>
									{removeUniquePrefix(tool.name)}
								</span>
							))
						) : (
							<span className="text-xs text-axiom-fg-3">No tools available</span>
						)}
					</div>
				</div>
			)}
			...
			{isOn && server.command && (
				<div className="mt-3">
					<div className="text-xs text-axiom-fg-3 mb-1">Command:</div>
					<div className="px-2 py-1 bg-axiom-bg-2 text-xs font-mono overflow-x-auto whitespace-nowrap text-axiom-fg-2 rounded-sm">
						{server.command}
					</div>
				</div>
			)}
...
const MCPServersList = () => {
	...
	if (mcpServiceState.error) {
		content = <div className="text-axiom-fg-3 text-sm mt-2">
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'axiom-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(axiomSettingsService.state, null, 2)
			downloadName = 'axiom-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					axiomSettingsService.dangerousSetState(json as any)
				}
	...
	return (
		<div className={`@@axiom-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
          ...
          ${selectedSection === tab
										...
										: 'bg-axiom-bg-2 hover:bg-axiom-bg-2/80 text-axiom-fg-1'}
        `}
							>
								{label}
							</button>
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`Axiom's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-axiom-fg-3 mb-2`}>{`Axiom can access any model that you host locally. We automatically detect your local models by default.`}</h3>

									<div className='opacity-80 mb-4'>
										<OllamaSetupInstructions sayWeAutoDetect={true} />
									</div>

									<AxiomProviderSettings providerNames={localProviderNames} />
								</ErrorBoundary>
							</div>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-axiom-fg-3 mb-2`}>{`Axiom can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>

									<AxiomProviderSettings providerNames={nonlocalProviderNames} />
								</ErrorBoundary>
							</div>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										<ErrorBoundary>
											...
											<div>
												...
												<div className='text-sm text-axiom-fg-3 mt-1'>
													<span>
														Experimental.{' '}
													</span>
													<span
														...
														data-tooltip-class-name='axiom-max-w-[20px]'
													>
												...
												<div className='my-2'>
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<AxiomSwitch
																...
																onChange={(newVal) => axiomSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
															<span className='text-axiom-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`my-2 ${!settingsState.globalSettings.enableAutocomplete ? 'hidden' : ''}`}>
															<ModelDropdown featureName={'Autocomplete'} className='text-xs text-axiom-fg-3 bg-axiom-bg-1 border border-axiom-border-1 rounded p-0.5 px-1' />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-axiom-fg-3 mt-1'>Settings that control the behavior of the Apply button.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<AxiomSwitch
															...
															onChange={(newVal) => axiomSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
														<span className='text-axiom-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncApplyToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'Apply'} className='text-xs text-axiom-fg-3 bg-axiom-bg-1 border border-axiom-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
										...
										<div>
											...
											<div className='text-sm text-axiom-fg-3 mt-1'>{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<AxiomSwitch
															...
															onChange={(newVal) => axiomSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
														<span className='text-axiom-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<AxiomSwitch
															...
															onChange={(newVal) => axiomSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
														<span className='text-axiom-fg-3 text-xs pointer-events-none'>Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-axiom-fg-3 mt-1'>{`Settings that control the visibility of Axiom suggestions in the code editor.`}</div>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<AxiomSwitch
															...
															onChange={(newVal) => axiomSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
														<span className='text-axiom-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className='w-full'>
												...
												<div className='text-sm text-axiom-fg-3 mt-1'>Settings that control the behavior of the commit message generator.</div>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<AxiomSwitch
															...
															onChange={(newVal) => axiomSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
														<span className='text-axiom-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`my-2 ${settingsState.globalSettings.syncSCMToChat ? 'hidden' : ''}`}>
														<ModelDropdown featureName={'SCM'} className='text-xs text-axiom-fg-3 bg-axiom-bg-1 border border-axiom-border-1 rounded p-0.5 px-1' />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-axiom-fg-3 mb-4'>{`Transfer your editor settings into Axiom.`}</h4>
								...
								<div>
									...
									<h4 className='text-axiom-fg-3 mb-4'>{`Transfer Axiom's settings and chats in and out of Axiom.`}</h4>
									<div className='flex flex-col gap-8'>
										{/* Settings Subcategory */}
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											<input key={2 * s} ref={fileInputSettingsRef} type='file' accept='.json' className='hidden' onChange={handleUpload('Settings')} />
											<AxiomButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputSettingsRef.current?.click() }}>
												Import Settings
											</AxiomButtonBgDarken>
											<AxiomButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Settings')}>
												Export Settings
											</AxiomButtonBgDarken>
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { axiomSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<AxiomButtonBgDarken className='px-4 py-1 w-full' onClick={() => { fileInputChatsRef.current?.click() }}>
												Import Chats
											</AxiomButtonBgDarken>
											<AxiomButtonBgDarken className='px-4 py-1 w-full' onClick={() => onDownload('Chats')}>
												Export Chats
											</AxiomButtonBgDarken>
								...
								<div>
									...
									<h4 className={`text-axiom-fg-3 mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className='flex flex-col gap-2 justify-center max-w-48 w-full'>
											<AxiomButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openSettings') }}>
												General Settings
											</AxiomButtonBgDarken>
											<AxiomButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.openGlobalKeybindings') }}>
												Keyboard Settings
											</AxiomButtonBgDarken>
											<AxiomButtonBgDarken className='px-4 py-1' onClick={() => { commandService.executeCommand('workbench.action.selectTheme') }}>
												Theme Settings
											</AxiomButtonBgDarken>
											<AxiomButtonBgDarken className='px-4 py-1' onClick={() => { nativeHostService.showItemInFolder(environmentService.logsHome.fsPath) }}>
												Open Logs
											</AxiomButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-axiom-fg-3 mb-4`}>Axiom Editor community builds do not send product analytics or machine identifiers. AI requests are sent only to the model providers you configure.</h4>
								</div>

								{/* AI Instructions section */}
								<div className='max-w-[600px]'>
									<h2 className={`text-3xl mb-2`}>AI Instructions</h2>
									<h4 className={`text-axiom-fg-3 mb-4`}>
										<ChatMarkdownRender inPTag={true} string={`
System instructions to include with all AI requests.
Alternatively, place a \`.axiomrules\` file in the root of your workspace.
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<AxiomSwitch
													...
													onChange={(newValue) => {
														axiomSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
												<span className='text-axiom-fg-3 text-xs pointer-events-none'>
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className='text-axiom-fg-3 text-xs mt-1'>
											{`When disabled, Axiom will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className='my-2'>
										<AxiomButtonBgDarken className='px-4 py-1 w-full max-w-48' onClick={async () => { await mcpService.revealMCPConfigFile() }}>
											Add MCP Server
										</AxiomButtonBgDarken>
									</div>
