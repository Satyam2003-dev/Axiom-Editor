...
import { AXIOM_VIEW_CONTAINER_ID, AXIOM_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { AXIOM_TOGGLE_SETTINGS_ACTION_ID } from './axiomSettingsPane.js';
...
const AXIOM_OPEN_SIDEBAR_ACTION_ID = 'axiom.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: AXIOM_OPEN_SIDEBAR_ACTION_ID, title: localize2('axiomOpenSidebar', 'Axiom: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(AXIOM_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('axiomCmdL', 'Axiom: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(AXIOM_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(AXIOM_OPEN_SIDEBAR_ACTION_ID)
		}
...
const AXIOM_CMD_SHIFT_L_ACTION_ID = 'axiom.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AXIOM_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', AXIOM_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(AXIOM_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'axiom.settingsAction',
			title: `Axiom's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(AXIOM_TOGGLE_SETTINGS_ACTION_ID)
	}
})
