...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		@IAxiomModelService axiomModelService: IAxiomModelService,
		@IEditCodeService editCodeService: IEditCodeService,
		@ITerminalToolService private readonly terminalToolService: ITerminalToolService,
		@IAxiomCommandBarService private readonly commandBarService: IAxiomCommandBarService,
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				await axiomModelService.initializeModel(uri)
				const { model } = await axiomModelService.getModelSafe(uri)
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = axiomModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.axiomSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by Axiom after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}
