...
import { AXIOM_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, AxiomStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './axiomSettingsTypes.js';
...
export type AxiomSettingsState = {
...
// type RealAxiomSettings = Exclude<keyof AxiomSettingsState, '_modelOptions'>
// type EventProp<T extends RealAxiomSettings = RealAxiomSettings> = T extends 'globalSettings' ? [T, keyof AxiomSettingsState[T]] : T | 'all'


export interface IAxiomSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: AxiomSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: AxiomSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: AxiomStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, AxiomStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: AxiomSettingsState): AxiomSettingsState => {
...
const _validatedModelState = (state: Omit<AxiomSettingsState, '_modelOptions'>): AxiomSettingsState => {
...
export const IAxiomSettingsService = createDecorator<IAxiomSettingsService>('AxiomSettingsService');
class AxiomSettingsService extends Disposable implements IAxiomSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: AxiomSettingsState;
	...
	dangerousSetState = async (newState: AxiomSettingsState) => {
	...
	private async _readState(): Promise<AxiomSettingsState> {
		const encryptedState = this._storageService.get(AXIOM_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(AXIOM_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: AxiomSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: AxiomStatefulModelInfo[] = [
...
registerSingleton(IAxiomSettingsService, AxiomSettingsService, InstantiationType.Eager);
