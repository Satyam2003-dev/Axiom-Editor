...
import { MCPUserStateOfName } from './axiomSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('axiom-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.axiomSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.axiomSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.axiomSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.axiomSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.axiomSettingsService.setMCPServerState(serverName, { isOn });
