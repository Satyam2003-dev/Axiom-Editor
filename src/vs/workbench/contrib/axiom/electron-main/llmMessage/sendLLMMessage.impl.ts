...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/axiomSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		const thisConfig = settingsOfProvider[providerName]
		return new OpenAI({ baseURL: `${thisConfig.endpoint.replace(/\/v1\/?$/, '')}/v1`, apiKey: 'llama.cpp', ...commonPayloadOpts })
	}
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://github.com/Satyam2003-dev/Axiom-Editor', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'Axiom', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`Axiom providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Axiom: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Axiom if you want the default url).`)
